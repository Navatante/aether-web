...
/**
 * Filtra el personal basándose en búsqueda y rating seleccionado.
 */
export function filterPersonnel(
    ...
    return personnel.filter((personKey) => {
        const fullName = fullNameMap[personKey]?.toLowerCase() || personKey.toLowerCase();
        const matchesSearch =
...
/**
 * Verifica si un piloto tiene exclusión mutua (Model Ratings).
 * Los ratings 1-3 son mutuamente excluyentes.
 */
export function isPilotMutuallyExcluded(
    personKey: string,
    ratingId: number,
    certifications: CertificationData
): boolean {
    if (ratingId > 3) return false;

    const personCerts = certifications[personKey];
    if (!personCerts) return false;

    const otherBasicRatings = [1, 2, 3].filter(id => id !== ratingId);
    const hasOtherBasicRating = otherBasicRatings.some(id => personCerts[id]?.certified);

    return hasOtherBasicRating && !personCerts[ratingId]?.certified;
}
