// Cuerpo compartido de las páginas de Instrucción (Pilotos / Dotaciones).
// Ambas variantes son idénticas salvo configuración (título, roles, planes y
// selector de queryKey): se parametriza con `InstruccionVariant` y cada página
// es un wrapper fino que pasa su config.
...
export interface InstruccionVariant {
    /** Título de la página. */
    title: string;
    /** Roles que pide el endpoint (CSV). */
    roles: string;
    /** Planes que pide el endpoint (CSV). */
    planes: string;
    /** Selector de queryKey del factory (pilotos | dotaciones). */
    queryKey: (escId: number, args: Record<string, unknown>) => QueryKey;
}

export function InstruccionPage({ variant }: { variant: InstruccionVariant }) {
    const instruccionArgs = { roles: variant.roles, planes: variant.planes };
    ...
    const { data, isLoading, error: queryError, refetch } = useApiQuery<PapeletasPersonasData>(
        ...
        variant.queryKey(escId ?? 0, instruccionArgs),
    );
                            ...
                            {filteredPapeletas.length === 0 ? (
                                ...
                                filteredPapeletas.map((papeleta, idx) => {
                                    ...
                                    >
                                        ...
                                        <td className={stickyFirstColClass(idx, "p-4 min-w-0 whitespace-nowrap border-b border-border")}>
                                            ...
                                            <Tooltip>
                                                ...
                                                <TooltipContent
                                                    side="top"
                                                    sideOffset={8}
                                                    variant="info" className="max-w-80 p-3"
                                                >
                                        ...
                                        {/* Celdas de estado por persona - usa StatusCell memoizado con tooltip nativo */}
                                        {visiblePersons.map(persona => (
                                            <StatusCell
                                                key={persona.person_sk}
