...
import type { FormationResponse } from '@/types/hours'
import type { PredefinedRange, StatsParams } from '@/shared/components/common'

export interface UseHorasFormacionOptions {
...
export interface UseHorasFormacionResult {
    ...
    /** Horas de formación por persona (Día / GVN). */
    chartData: FormationResponse['tripulantes']
...
/**
 * Lógica de la vista de horas de vuelo en formación (operations.formation_hour):
 * estado del rango, consulta a `/hours/formation-period` vía TanStack Query.
...
export function useHorasFormacion(options: UseHorasFormacionOptions = {}): UseHorasFormacionResult {
    ...
    const {
        ...
        isFetching: loading,
        error: queryError,
    } = useApiQuery<FormationResponse>(
        'GET',
        '/hours/formation-period',
        { query: queryParams },
        queryKeys.hours.formacion(escId ?? 0, queryParams),
    )
