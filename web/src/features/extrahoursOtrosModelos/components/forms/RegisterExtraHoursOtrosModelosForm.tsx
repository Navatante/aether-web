// Formulario de alta/edición de Horas extra (solo render). La lógica vive en
// hooks/useRegisterExtraHoursOtrosModelos.
...
import { useRegisterExtraHoursOtrosModelos } from '../../hooks/useRegisterExtraHoursOtrosModelos';
import type { ExtraHourItem } from '@/types/generated/extrahours';

interface RegisterExtraHoursOtrosModelosFormProps {
    mode: 'create' | 'edit';
...
const HOUR_FIELDS = [
    ...
    { key: 'gvn', label: 'GVN' },
    { key: 'inst', label: 'Instrumentos' },
] as const;

export default function RegisterExtraHoursOtrosModelosForm({ mode, initial, onClose }: RegisterExtraHoursOtrosModelosFormProps) {
    const {
        person, setPerson,
        ...
        handleSubmit,
    } = useRegisterExtraHoursOtrosModelos({ mode, initial, onClose });

    // Mapa key → [valor, setter] para renderizar los inputs de horas en un bucle.
