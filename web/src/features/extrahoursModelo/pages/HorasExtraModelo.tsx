...
import type { PersonTotalsItem, ExtraModelHourItem } from '@/types/generated/extramodelhours';
import {
...
import { RegisterExtraModelHoursDialog } from '../components';
...
const HorasExtraModelo = () => {
    ...
    const {
        ...
        path: '/extra-model-hours',
        query,
        queryKey: queryKeys.extraModelHours.list(escId ?? 0, params),
    });
    ...
    const { data: detail, isLoading: detailLoading } = useApiQuery<ExtraModelHourItem[]>(
        'GET',
        `/extra-model-hours/person/${expanded ?? 0}`,
        { enabled: expanded != null },
        queryKeys.extraModelHours.byPerson(escId ?? 0, expanded ?? 0),
    );

    // Edición de un registro individual.
    const [editing, setEditing] = useState<ExtraModelHourItem | null>(null);
    ...
    const deleteRecord = useApiMutation<void, { id: number }>(
        'DELETE', (v) => `/extra-model-hours/${v.id}`,
        {
        ...
        <div className="h-full p-3 sm:p-6 pb-8 flex flex-col">
            <div className="w-full mx-auto flex flex-col flex-1 min-h-0">
                ...
                <div className="mb-8 text-center flex-shrink-0">
                    <GradientTitle>Horas extra NH-90</GradientTitle>
                </div>
                ...
                {/* Tabla agrupada por persona (totales real+sim). Click → despliega registros. */}
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Esta acción no se puede deshacer. Eliminará permanentemente el registro de horas de modelo{' '}
...
export default HorasExtraModelo;
