...
func (h *Handlers) Register(g *echo.Group, authSvc *auth.Service) {
	...
	g.GET("/extra-model-hours", h.List, mw)
	g.GET("/extra-model-hours/person/:id", h.ListByPerson, mw)
	g.POST("/extra-model-hours", h.Insert, mw, operacional)
	g.PUT("/extra-model-hours/:id", h.Update, mw, operacional)
	g.DELETE("/extra-model-hours/:id", h.Delete, mw, operacional)
}
...
func (h *Handlers) Insert(c echo.Context) error {
	...
	var data ExtraModelHourFormData
